...
class ZWYT(object):
    def __init__(self, name, username, password, periods, pushplus_token):
        ...
        logFile = Path(
            logDir
            / f"{datetime.now().year}-{datetime.now().month}-{datetime.now().day}.log"
        )
    ...
    def pushplus(self, title, content):
        ...
        Returns: 无
        """
        params = {"token": self.pushplus_token, "title": title, "content": content}
    ...
    def get_seat_resvDev_devSn(self, devName: str, tag: str):
        ...
        """
        resvDev = None
        filename = devName.strip().split("-")[
            0
        ]  # 移除传入的座位名头尾的空格后再分割传入的座位名称
        ...
        for i in json_data.get("data"):
            if (
                i.get("devName") == devName.upper()
                or i.get("devName") == devName.lower()
            ):
    ...
    def login(self):
        ...
        rsa = RSA().strEnc(
            self.username + self.password + lt
        )  # 把密码和那些参数用RSA加密
    ...
    def get_reserve_date(self) -> typing.List:
        ...
        t_year, t_month, t_day = two_day.year, two_day.month, two_day.day
    ...
    def reserve(self, devName: str):
        ...
        for date in self.get_reserve_date():
            ...
            # 预约失败---可选择向微信推送预约失败的信息, 比如可以使用 pushplus 平台
                ...
                logger.error(
                    f"{self.name} 时间段: {json_data['resvBeginTime']} 预约失败 {message}"
                )
